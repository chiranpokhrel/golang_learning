...
2. [The Unix System at UiS](#the-unix-system-at-uis)
...
7. [Remote Login with Secure SHell](#remote-login-with-secure-shell)
...
If you are running Windows, please consult the instructions [here](https://github.com/dat520-2024/info/blob/main/setup-wsl.md).

However, we will also be using some machines on our Unix system, named `gorina1` - `gorina3`, through remote access; see below for more information about this.
All necessary software for this course should be installed on these machines.

Some of the later assignments includes a networking part, requiring you to run your code on several machines.
...
This makes it easy to run and test the example code and your project later.
...
Other machines (jump hosts) that are accessible from the Internet, e.g. from home, are: `ssh2`, `ssh3`.

This must be done before you can login to other Unix machines on campus.
...
*The following may be skipped if you can login from one gorina machine to another without typing a password.*
...
You can avoid having to type the password each time by generating a public-private key-pair using the `ssh-keygen` command.
...
[here](https://foswiki.ux.uis.no/bin/view/Info/WebHome).
...
Below we use `git@github.com:dat520-2024/username-labs.git` as an example.

```console
# clone the Git repo with SSH into the $HOME/assignments directory
git clone git@github.com:dat520-2024/username-labs.git assignments
cd assignments
git remote add course-assignments git@github.com:dat520-2024/assignments.git 
...
Read our [lab submission guide](https://github.com/dat520-2024/info/blob/main/lab-submission.md) for more detailed instructions on how to submit your assignments to be evaluated by QuickFeed.
